...
class State:
    ...
    def heuristic(self):
        ...
        return dist(
            ...
            (State.goal['x'], State.goal['y'])) / MAX_TRAM_SPEED
...
class CityMap:
    ...
    def search(self, start, goal, time_of_beginning):
        ...
        while current.get_stop_code() != goal["code"]:
            for one in self.get_neighbors_codes(current.get_stop_code()):
                cost = current.get_time() + self.fastest_transition(
                    current.get_stop_code(), one, current.get_time())
                to_visit.put(State(self.get_stop(one), cost, current))
