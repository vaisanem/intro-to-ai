...
class Perceptron:
    ...
    def __init__(self, images, chars):
        ...
        self.weights = [0]*NUMBER_OF_PIXELS

        self.data = [{'vector': v, 'char': c} for (v, c) in zip(idata, cdata)]
        random.seed()

    def train(self, target_char, opposite_char, steps):
        d = [x for x in self.data[:5000] if x['char'] in (target_char, opposite_char)]
        e = self.error(d, target_char, opposite_char)
        #s = 0
        #while e >= 0 and s < steps:
        while e >= 0:
            #e = random.randrange(len(d))
            res = np.dot(d[e]["vector"], self.weights)
            scalar = 1
            if res >= 0:
                scalar = -1
            self.weights = np.add(self.weights, np.multiply(scalar, d[e]["vector"])).tolist()
            e = self.error(d, target_char, opposite_char)
            #s += 1

    def error(self, data, target_char, opposite_char):
        e = -1
        for i in range(len(data)):
           res = np.dot(data[i]["vector"], self.weights)
           if (res >= 0 and data[i]["char"] != target_char) or (res < 0 and data[i]["char"] != opposite_char):
               e = i
               break
        return e

    def activate(self, value, target_char, opposite_char):
        if value >= 0:
            return target_char
        else: 
